...
class Battle extends Phaser.Scene{
	...
	createEnemy(){
		...
		else if(currentEnemy === 'Officer' || currentEnemy === 'Soldier4r3'){
			if(currentEnemy === 'Officer'){
				this.eName = 'Officer';
			}
			else{
				this.eName = 'Soldier';
		...
		else if(currentEnemy === 'GangLeader'){
			this.createGangLeader();
		}
		this.enemy.body.setAllowGravity(false);
		this.add.text(90, 50, this.name + ' LV.' + this.enemyStats.level, { fontSize: '30px', fill: '#001', fontFamily: 'Impact' });
	}
	createEdgel0rd(){
	...
	createOfficer(){
		this.enemy = this.physics.add.sprite(400,150, 'officer');
		OfficerAnims(this);
		this.enemy.anims.play('OfficerIdle');
		this.enemyObject = new Officer();
		this.enemyStats = new Character(50, this.eName, 500, 6, 20);
		this.name = this.eName;
	}
	createGangLeader(){
		this.enemy = this.physics.add.sprite(400,150, 'gangLeaderB');
		gangAnims(this);
		this.enemy.anims.play('gangIdle');
		this.enemyObject = new GangLeader();
		this.enemyStats = new Character(50, 'Gang Leader', 300, 6, 20);
		this.name = 'Gang Leader';
	}
	...
	createAlly(){
		if(AllyTeam.Ally1.health <= 0){//TEMPORARY *need to add mechanic to heal outside of battle + gameover interactions
			AllyTeam.Ally1.health = 100;
		}
	...
	updateBoss(){
		this.bossBar.setCrop(0, 0, 181 * (this.enemyStats.health / 500), 600);
	}
	updateAlly(){
		this.allyHp.setCrop(0, 0, 128 * (AllyTeam.Ally1.health / 100), 600);
	...
	checkGameOver(){
		...
		if(!this.textActive && this.gameOver && this.endText){
	...
	setVictory(){
		...
		else if(currentEnemy === 'Officer'){
			officerDefeated = true;
		}
		else if(currentEnemy === 'Soldier4r3'){
			s4r3Clear = true;
			s4r3SoldiersDefeated = true;
		}
		else if(currentEnemy === 'GangLeader'){
			Stage4r4Clear = true;
		}
	...
	//change to a different scene after leaving depending on the scene
	checkLastStage(){
		if(lastStage === 'Stage4r4'){
			lastStage = 'Stage4r2';
		}
	}
}
